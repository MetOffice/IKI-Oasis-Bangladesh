...
Note: Python 3 compatible only. Use scitools/default-current.
...
def process_storm(dfrow):
    ...
    fig, axs = plt.subplots(2, 1, figsize=(7, 5), sharex='col')
    ...
    # Axis labels
    axs[0].text(0.03, 0.9, 'Maximum Wind/Gust Speed', color='darkgrey', fontsize=10, ha='left', weight='bold',
                transform=axs[0].transAxes)
    axs[1].text(0.03, 0.9, 'Minimum Mean Sea-level Pressure', color='darkgrey', fontsize=10, ha='left', weight='bold',
                transform=axs[1].transAxes)
    # Gust speed plotting
    l5 = axs[0].plot(era5.index, era5.GUST, 'k-', lw=2, zorder=200)
    axs[0].errorbar(ibtracs.index - pd.Timedelta(minutes=20), ibtracs.USA_WIND, yerr=werror, fmt='<', ms=4,
                    zorder=100, label="IBTrACS US", color='grey', mfc='darkgrey', lw=1)
    axs[0].errorbar(ibtracs.index + pd.Timedelta(minutes=20), ibtracs.NEWDELHI_WIND, yerr=werror, fmt='>', ms=4,
    ...
    # MSLP plotting
    l5 = axs[1].plot(era5.index, era5.PRES, 'k-', lw=2, label='ERA5', zorder=200)
    # axs[1].text(endtime + pd.Timedelta(hours=1), era5.PRES[-1] / 100, 'ERA5', va='center', weight='bold')
    axs[1].errorbar(ibtracs.index - pd.Timedelta(minutes=20), ibtracs.USA_PRES, yerr=perror, fmt='<', ms=4,
                    zorder=100, label="IBTrACS US", color='grey', mfc='darkgrey', lw=1)
    axs[1].errorbar(ibtracs.index + pd.Timedelta(minutes=20), ibtracs.NEWDELHI_PRES, yerr=perror, fmt='>', ms=4,
                    zorder=100, label="IBTrACS ND", color='grey', mfc='darkgrey', lw=1)
    for i in range(9):
        l4 = axs[1].plot(pres44.index, pres44[i], lw=3, color=psl_color_cycle[i], label='_nolegend_')
        axs[1].plot(pres44.index, pres44[i], lw=1, color='white', label='_nolegend_')
    axs[1].legend(frameon=False, fontsize=8, loc=4)
    axs[1].xaxis.set_major_formatter(mdates.DateFormatter('%b-%d\n%HZ'))
    plt.tight_layout()
    # plt.show()
    fig.savefig(f'compare{dfrow.NAME}.png', dpi=600)
    plt.close()
...
IBTRACSFILE = 'ibtracs.NI.list.v04r00.csv'

# Load Validation area shapefile
val = shape.load_shp('ValidationArea2.shp')
